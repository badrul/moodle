...
// Minimum number of digits shown (zero-padding). Set to 0 to disable and format the number with grouped thousands
...
if (file_exists($logfile))
{
	...
	if ($count_unique==0 || !isset($_COOKIE[$cname]))
    {
		...
		$count = $count + 1;
		$fp = @fopen($logfile,'w+') or die('// ERROR: Can\'t write to the log file ('.$page.'), please make sure this file exists and is CHMOD to 666 (rw-rw-rw-)!');
		flock($fp, LOCK_EX);
		fputs($fp, $count);
		flock($fp, LOCK_UN);
		fclose($fp);
    ...
    echo 'document.write(\'Total Unique Visitors: '.$count.'\');';
    exit();
}
else
{
...
/* This function handles input parameters making sure nothing dangerous is passed in */
function input($in)
{
