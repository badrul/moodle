...
if($certificate->printdate > 0)    {
	...
	$certificatedate =str_replace('January','Januari',$certificatedate);
	$certificatedate =str_replace('February','Februari',$certificatedate);
	$certificatedate =str_replace('Mac','Mars',$certificatedate);
	$certificatedate =str_replace('May','Mei',$certificatedate);
	$certificatedate =str_replace('June','Jun',$certificatedate);
	$certificatedate =str_replace('July','Julai',$certificatedate);
	$certificatedate =str_replace('August','Ogos',$certificatedate);
	$certificatedate =str_replace('October','Oktober',$certificatedate);
	$certificatedate =str_replace('December','Disember',$certificatedate);
	$certificatedate =get_string('onms', 'certificate').' '.$certificatedate;
}
...
// Add text
	print_seal('akpk.png', $orientation, 125, 20, 55, 28);
    $pdf->SetTextColor(0,0,120);
    cert_printtext($x, $y, 'C', 'Helvetica', '', 30, get_string('titlems', 'certificate'));
    $pdf->SetTextColor(0,0,0);
    cert_printtext($x, $y+20, 'C', 'Helvetica', '', 18, get_string('certifyms', 'certificate'));
    cert_printtext($x, $y+30, 'C', 'Helvetica', '', 30, $studentname);
    cert_printtext($x, $y+48, 'C', 'Helvetica', '', 18, get_string('statementms', 'certificate'));
	print_seal('powerms.png', $orientation, 120, $y+63,'', '');
    //cert_printtext($x, $y+77, 'C', 'Helvetica', '', 18, get_string('programme','certificate'));
    cert_printtext($x, $y+85, 'C', 'Helvetica', '', 18, $certificatedate);
