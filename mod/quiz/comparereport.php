...
// This script uses installed report plugins to print quiz reports
    ...
    $url = new moodle_url('/mod/quiz/comparereport.php', array('id' => $cm->id));
    if ($mode !== '') {
        $url->param('mode', $mode);
    }
    $PAGE->set_url($url);
	...
	if($otherquizs=$DB->get_records_sql("SELECT a.* FROM {course_modules} a".
           " WHERE a.section = $cm->section AND a.module = $cm->module AND a.instance!=$cm->instance AND a.course = $cm->course")){
		foreach ($otherquizs as $otherquiz) {
			if($otherquiz->instance>$quiz->id){
				$quiz2 = $DB->get_record('quiz', array('id' => $otherquiz->instance));
			}else{
				$quiz2 = $quiz;
				$quiz = $DB->get_record('quiz', array('id' => $otherquiz->instance));
			}
		}
	}
    ...
    add_to_log($course->id, "quiz", "report", "comparereport.php?id=$cm->id", "$quiz->id", "$cm->id");
...
/// Open the selected quiz report and display it
    ...
    if (!$report->display2($quiz, $cm,$quiz2, $otherquiz, $course)) { // Run the report!
